...
![](images/antlr4/old/decision_tree_feature_importance.png)
![](images/antlr4/old/decision_tree_visualization.png)
#### 決定木
...
![](images/ceylon-ide-eclipse/old/cv_performance_chart.png)
![](images/ceylon-ide-eclipse/old/feature_importance_chart.png)
#### 特徴量重要度
...
![](images/ceylon-ide-eclipse/old/feature_histograms.png)
#### 特徴量の分布
- Maintainability Index関連のメトリクスは分散が大きい
- ハルステッドメトリクス、コード行数やトークン数の変化量は分散が比較的小さいため、それぞれの特徴量のみでクラスを分類するのは難しい
![](images/ceylon-ide-eclipse/old/partial_dependence_plots.png)
#### Partial Dependence Plot
...
![](images/ceylon-ide-eclipse/old/decision_tree_feature_importance.png)
![](images/ceylon-ide-eclipse/old/decision_tree_visualization.png)
#### 決定木
...
![](images/elasticsearch/old/cv_performance_chart.png)
![](images/elasticsearch/old/feature_importance_chart.png)
#### 特徴量重要度
- トークン数やコード行数の変化量の重要度が高い
- メソッド操作が「追加」であるかどうかもクラス分類の判断に影響を与えている
- ハルステッドメトリクス、Maintainability Index関連のメトリクスもある程度の重要度がある
![](images/elasticsearch/old/feature_histograms.png)
#### 特徴量の分布
- トークン数、コード行数、循環的複雑度のいずれも変化量はほとんど0
![](images/elasticsearch/old/partial_dependence_plots.png)
#### Partial Dependence Plot
- トークン数、コード行数の変化量が0付近であるほど陽性クラスの予測確率が高まる
- MISM、MISEIは凹型であり、0付近で陽性クラスの予測確率が最も低くなる
![](images/elasticsearch/old/decision_tree_feature_importance.png)
![](images/elasticsearch/old/decision_tree_visualization.png)
#### 決定木
...
![](images/BroadleafCommerce/old/cv_performance_chart.png)
![](images/BroadleafCommerce/old/feature_importance_chart.png)
#### 特徴量重要度
- ハルステッドメトリクス、Maintainability Index関連のメトリクスの重要度が高い
- 変化量メトリクスは特徴量の重要度上位20件に含まれていない
![](images/BroadleafCommerce/old/feature_histograms.png)
#### 特徴量の分布
- MI、MISEI、MISM、MIMSに明確な二峰性があり、陽性クラスや陰性クラスの傾向を掴みにくい
- ハルステッドメトリクスの値が全体的に大きく、陽性クラスと陰性クラスの境界を設定しにくい
![](images/BroadleafCommerce/old/partial_dependence_plots.png)
#### Partial Dependence Plot
- ハルステッドメトリクスが一定の値以上になると陽性クラスの予測確率が高止まりする
	- ハルステッドメトリクスは重要度が高いが、数値的な重要度はあまり高くないことを示唆している
![](images/BroadleafCommerce/old/decision_tree_feature_importance.png)
![](images/BroadleafCommerce/old/decision_tree_visualization.png)
#### 決定木
...
![](images/hazelcast/old/cv_performance_chart.png)
![](images/hazelcast/old/feature_importance_chart.png)
#### 特徴量重要度
- トークン数やコード行数の変化量の重要度が高い
- ハルステッドメトリクスやMaintainability Index関連のメトリクスも重要度が高い
![](images/hazelcast/old/feature_histograms.png)
#### 特徴量の分布
- ハルステッドメトリクス、トークン数やコード行数の変化量の分布は大きな偏りがある
![](images/hazelcast/old/partial_dependence_plots.png)
#### Partial Dependence Plot
...
![](images/hazelcast/old/decision_tree_feature_importance.png)
![](images/hazelcast/old/decision_tree_visualization.png)
#### 決定木
...
![](images/oryx/old/cv_performance_chart.png)
![](images/oryx/old/feature_importance_chart.png)
#### 特徴量重要度
- トークン数やコード行数の変化量の重要度が高い
...
![](images/oryx/old/feature_histograms.png)
#### 特徴量の分布
- コメント関連のメトリクスやMaintainability Index関連のメトリクスは分散が大きい
- トークン数やコード行数の変化量、ハルステッドメトリクスは分散が小さい
![](images/oryx/old/partial_dependence_plots.png)
#### Partial Dependence Plot
- コメント関連のメトリクスが一定以上のときに陽性クラスの予測確率が高止まりしている
- トークン数やコード行数の変化量については、0付近で陽性クラスの予測確率が最も高くなっている
![](images/oryx/old/decision_tree_feature_importance.png)
![](images/oryx/old/decision_tree_visualization.png)
#### 決定木
